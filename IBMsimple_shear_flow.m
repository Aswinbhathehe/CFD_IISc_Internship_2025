...
for n=1:tsteps
    ...
    [uL, vL]=vel_interpolate(u,v, Xb,Yb, dx,dy);
    epsilon=1;
    FxL=epsilon*(0-uL); 
    FyL=epsilon*(0-vL);
    ...
    FxL_old=FxL; FyL_old=FyL;

    forceField=spread_force(FxL,FyL, Xb,Yb, dx,dy,x,y);
    fx=forceField(:,:,1); 
    fy=forceField(:,:,2);

    u_center=0.5*(u(2:end-1, 2:end-1)+u(2:end-1, 3:end));
    fx_center=0.5*(fx(2:end-1,2:end-1)+fx(2:end-1,3:end));
    u_center_new=implicit_diffusion_u(u_center+dt*fx_center, Lx,  Ly, x, y, dt, visc);
    u(2:end-1, 2:end-1)=u_center_new; 
    u(2:end-1, 3:end)=u_center_new;
    ut=u;
    ...
    for cycle=1:20
        ...
        if norm(p(:)-p_old(:), 2)/norm(p_old(:), 2) < 1e-6
            break;
        end
    end
    ...
    if mod(n,10) == 0
        ...
        subplot(1,2,2);
        semilogy(1:n, err_l2_hist(1:n), 'k-', 'LineWidth', 2);
...
function u_new=implicit_diffusion_u(u_old, Lx, Ly, Nx, Ny, dt, nu)
    ...
    for j=1:Ny
        for i=1:Nx
            n=index(i,j);
            % Top and bottom wlls → Dirichilet BC
            if j == 1
                A(n,:)=0;
                A(n,n)=1;
                b(n)=0;       % Bottom wall velocity Ub
                continue;
            elseif j == Ny
                A(n,:)=0;
                A(n,n)=1;
                b(n)=10;      % Top wall velocity Ut
                continue;
            end
            % Interior and Left/Right
            A(n,n)=coeff_center;
            %Westside
            if i > 1
...
function p=V_cycle(p, rhs, Lx, Ly, Nx, Ny)
    ...
    res=compute_residual(p, rhs, Lx, Ly, Nx, Ny);
    ...
    p=sor_solve(p,rhs, Lx, Ly, Nx, Ny, 7);
end
...
function res=compute_residual(p, rhs, Lx, Ly, Nx, Ny)
    dx=Lx/Nx;
    dy=Ly/Ny;
    ...
    for i=2:Nx+1
        for j=2:Ny+1
            laplace=(p(j,i+1)-2*p(j,i)+p(j,i-1))/dx^2+  (p(j+1,i)-2*p(j,i)+p(j-1,i))/dy^2;
